...
configfile: "workflows/all_reefcompare.yml"
RC_SCANS = list(config['rc_scans'])
RC_ROIS = [ROI for ROI in ALL_ROIS
           if any(sub in ROI for sub in RC_SCANS)]
RC_ROI_IDs = [s.split('--')[0] for s in RC_ROIS]
RC_LABELS = [s.split('--')[1] for s in RC_ROIS]
RC_ROI_SCANS = ['-'.join(s.split('-')[:2]) for s in RC_ROI_IDs]

#roi_records = pd.read_csv("data/interim/all_ROIs_flat.csv")
scan_records = pd.read_csv(SCAN_RECORDS_PATH)

rule rc_spatial_resample:
    input:
        nc_file="data/interim/scans/{roi_scan_ID}/ROIs/{refl_type}/{roi}--{label}.nc"
    output:
        csv_file="data/interim/03_reefcompare/{refl_type}/{label}/{roi_scan_ID}/{roi}_blocks.csv",
    params:
        methods=['bilinear'],
...
rule rc_blocks_spectra_plot:
    input:
        csv_file="data/interim/03_reefcompare/{refl_type}/{label}/{roi_scan_ID}/{roi}_blocks.csv",
    output:
        csv_file="data/interim/03_reefcompare/{refl_type}/{label}/{roi_scan_ID}/{roi}_spectra.csv"
    params:
        figsize=(12, 6),
        dpi=300,
    run:
...
rule rc_compile_refl_summary:
    input:
        # Recursively find all txt files in results directory
        files=lambda wildcards: list(Path("data/interim/03_reefcompare/03_reflectance").rglob("*/*_blocks.csv"))
    output:
        csv_file="data/interim/03_reefcompare/03_reflectance-blocks.csv"
    run:
        list_of_dfs = []
...
rule rc_compile_normrefl_summary:
    input:
        # Recursively find all txt files in results directory
        files=lambda wildcards: list(Path("data/interim/03_reefcompare/03A_norm_refl").rglob("*/*_blocks.csv"))
    output:
...
rule create_rgb_refl_blocks_reefcompare:
    input:
        ...
        # Recursively find all txt files in results directory
        nc_files=lambda wildcards: list(Path("data/interim/03_reefcompare/03A_norm_refl").rglob("*/*_bilinear-1x1.nc"))
    output:
