...
rule rc_plot_second_deriv:
    ...
    output:
        jpg_file="data/interim/03_reefcompare/{refl_type}_2nd_dx/{label}/{roi_scan_ID}/{roi_block}_2nd_dx.jpg"
    params:
        figsize=(12, 12),
        dpi=300,
    run:
...
rule rc_spect_var_trio:
    ...
    output:
        nc_file="data/interim/03_reefcompare/{spectrum_type}/04A_spec_var/{label}/{roi_scan}/{roi_block}_trio.nc"
    params:
        band_start=7,# 421.3802 nm
        band_end=141  # 709.5606 nm
    run:
...
rule rc_spect_var_maps:
    input:
        nc_file="data/interim/03_reefcompare/{spectrum_type}/04A_spec_var/{label}/{roi_scan}/{roi_block}_trio.nc"
    output:
        img_file="data/interim/03_reefcompare/{spectrum_type}/04A_spec_var/{label}/{roi_scan}/{roi_block}_trio.png"
    params:
        figsize=(18, 7),
        dpi=300
    run:
...
rule rc_spect_var_trio_distr:
    input:
        nc_file="data/interim/03_reefcompare/{spectrum_type}/04A_spec_var/{label}/{roi_scan}/{roi_block}_trio.nc",
        img_file="data/interim/03_reefcompare/{spectrum_type}/04A_spec_var/{label}/{roi_scan}/{roi_block}_trio.png"
    output:
        img_file="data/interim/03_reefcompare/{spectrum_type}/04A_spec_var/{label}/{roi_scan}/{roi_block}_trio_distr.jpg"
    params:
        figsize=(18, 5),
        dpi=300
    run:
...
rule reefcompare_blocks_extract:
    input:
        ...
        #     refl_type=['03A_norm_refl', '03B_L2_norm_refl']),
        # expand("data/interim/03_reefcompare/{refl_type}/04C_spec_diversity/{roi_path}_specdiv/",
        #        roi_path=expand("{label}/{roi_scan_ID}/{roi_ID}_bilinear-1x1", zip,
        #        label=RC_LABELS, roi_scan_ID=RC_SCANS, roi_ID=RC_ROIS),
        #     refl_type=['03A_norm_refl', '03B_L2_norm_refl']),
