...
ss_roi_blocks = pd.read_csv("data/interim/02_seasim_ts/03A_norm_refl-blocks.csv")
ss_roi_blocks = ss_roi_blocks.loc[ss_roi_blocks['resampling_method'] == 'bilinear']
ss_roi_blocks = ss_roi_blocks.loc[ss_roi_blocks['block_grid'].isin(selected_block)]
ss_roi_blocks = ss_roi_blocks.loc[ss_roi_blocks['n_complete_blocks'] > 0]
SS_BLOCKS = ss_roi_blocks['filestem'].tolist()
SS_LABELS = ss_roi_blocks['label'].tolist()
SS_ROIS = ss_roi_blocks['roi_ID'].tolist()
SS_SCANS = ['-'.join(s.split('-')[:2]) for s in SS_ROIS]

selected_labels = ['brown_algae', 'dictyota', 'green_algae', 'green_brown_mix',
                   'lobophora', 'red_algae', 'plug_colonised', 'pvc_tray']
selected_roi_blocks = ss_roi_blocks.loc[ss_roi_blocks['label'].isin(selected_labels)]
...
timeseries_blocks = ss_roi_blocks.loc[~ss_roi_blocks['label'].isin(selected_labels)]
TS_BLOCKS = timeseries_blocks['filestem'].tolist()
TS_LABELS = timeseries_blocks['label'].tolist()
TS_ROIS = timeseries_blocks['roi_ID'].tolist()
TS_SCANS = ['-'.join(s.split('-')[:2]) for s in TS_ROIS]

rule ts_blocks_summarised:
    input:
        csv_file="data/interim/02_seasim_ts/03A_norm_refl-blocks.csv",
    output:
        csv_file="data/interim/02_seasim_ts/03A_norm_refl-blocks-summarised.csv"
    run:
...
rule ts_plot_spectrum:
    input:
        nc_file="data/interim/02_seasim_ts/{refl_only}_refl/{label}/{roi_scan_ID}/{roi_block}.nc"
    output:
        jpg_file="data/interim/02_seasim_ts/{refl_only}_refl/{label}/{roi_scan_ID}/{roi_block}_refl.jpg"
    params:
        band_start=7,
        band_end=141,
        figsize=(12, 6),
...
rule ts_spect_var_trio:
    input:
        nc_file="data/interim/02_seasim_ts/{spectrum_type}/{label}/{roi_scan}/{roi_block}.nc"
    output:
...
rule ts_spectral_PCA:
    input:
        nc_file="data/interim/02_seasim_ts/{spectrum_type}/{label}/{roi_scan}/{roi_block}.nc"
    output:
        scores="data/interim/02_seasim_ts/{spectrum_type}/04B_PCA/{label}/{roi_scan}/{roi_block}_PCA_scores.nc",
        loadings="data/interim/02_seasim_ts/{spectrum_type}/04B_PCA/{label}/{roi_scan}/{roi_block}_PCA_loadings.nc"
    params:
        #keep_variance=0.99,
        n_components=5,
        band_start=7,# 421.3802 nm
...
rule ts_spectral_PCA_jpg:
    input:
        scores="data/interim/02_seasim_ts/{spectrum_type}/04B_PCA/{label}/{roi_scan}/{roi_block}_PCA_scores.nc",
        loadings="data/interim/02_seasim_ts/{spectrum_type}/04B_PCA/{label}/{roi_scan}/{roi_block}_PCA_loadings.nc",
    output:
        pca_rgb="data/interim/02_seasim_ts/{spectrum_type}/04B_PCA/{label}/{roi_scan}/{roi_block}_PCA_rgb.png"
    params:
        dpi=300,
    run:
        import matplotlib.pyplot as plt
...
rule ts_PCA_var_contr:
    input:
        scores="data/interim/02_seasim_ts/{spectrum_type}/04B_PCA/{label}/{roi_scan}/{roi_block}_PCA_scores.nc",
        loadings="data/interim/02_seasim_ts/{spectrum_type}/04B_PCA/{label}/{roi_scan}/{roi_block}_PCA_loadings.nc",
        pca_rgb="data/interim/02_seasim_ts/{spectrum_type}/04B_PCA/{label}/{roi_scan}/{roi_block}_PCA_rgb.png",
    output:
        pca_var_contr="data/interim/02_seasim_ts/{spectrum_type}/04B_PCA/{label}/{roi_scan}/{roi_block}_PCA_var_contr.png"
    params:
...
rule ts_spect_diversity:
    input:
        scores="data/interim/02_seasim_ts/{spectrum_type}/04B_PCA/{label}/{roi_scan}/{roi_block}_PCA_scores.nc",
    output:
        csv_file="data/interim/02_seasim_ts/{spectrum_type}/04C_spec_diversity/{label}/{roi_scan}/{roi_block}_specdiv.csv",
        output_dir=directory("data/interim/02_seasim_ts/{spectrum_type}/04C_spec_diversity/{label}/{roi_scan}/{roi_block}_specdiv")
    params:
...
rule ts_spectdiv_plots:
    input:
        csv_file="data/interim/02_seasim_ts/{spectrum_type}/04C_spec_diversity/{label}/{roi_scan}/{roi_block}_specdiv.csv",
        scores="data/interim/02_seasim_ts/{spectrum_type}/04B_PCA/{label}/{roi_scan}/{roi_block}_PCA_scores.nc",
    output:
        png_file="data/interim/02_seasim_ts/{spectrum_type}/04C_spec_diversity/{label}/{roi_scan}/{roi_block}_specdiv.png",
    params:
        dpi=300
    run:
        import matplotlib.pyplot as plt
...
rule timeseries_blocks_extract:
    input:
        ...
        # expand("data/interim/02_seasim_ts/{refl_type}/{roi_path}_refl.jpg",
        ...
        #         label=SS_LABELS,roi_scan_ID=SS_SCANS,roi_ID=SS_ROIS),
        #     refl_type=['03A_norm_refl', '03B_L2_norm_refl']),
        # expand("data/interim/02_seasim_ts/{refl_type}/04C_spec_diversity/{roi_path}_specdiv/",
        #     roi_path=expand("{label}/{roi_scan_ID}/{roi_ID}_bilinear-1x1",zip,
        #         label=SS_LABELS,roi_scan_ID=SS_SCANS,roi_ID=SS_ROIS),
        #     refl_type=['03A_norm_refl', '03B_L2_norm_refl']),
