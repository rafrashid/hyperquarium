...
configfile: "workflows/all_timeseries.yml"
TS_SCANS = list(config['ts_scans'])
TS_ROIS = [ROI for ROI in ALL_ROIS
           if any(sub in ROI for sub in TS_SCANS)]
...
rule ts_spatial_resample:
    input:
        nc_file="data/interim/scans/{roi_scan_ID}/ROIs/{refl_type}/{roi}--{label}.nc"
    output:
        csv_file="data/interim/02_seasim_ts/{refl_type}/{label}/{roi_scan_ID}/{roi}_blocks.csv",
    params:
        methods=['bilinear'],
...
rule ts_blocks_spectra_plot:
    input:
        csv_file="data/interim/02_seasim_ts/{refl_type}/{label}/{roi_scan_ID}/{roi}_blocks.csv",
    output:
        csv_file="data/interim/02_seasim_ts/{refl_type}/{label}/{roi_scan_ID}/{roi}_spectra.csv"
    params:
        figsize=(12, 6),
        dpi=300,
    run:
...
rule ts_compile_refl_summary:
    input:
        # Recursively find all txt files in results directory
        files=lambda wildcards: list(Path("data/interim/02_seasim_ts/03_reflectance").rglob("*/*_blocks.csv"))
    output:
        csv_file="data/interim/02_seasim_ts/03_reflectance-blocks.csv"
    run:
        list_of_dfs = []
...
rule ts_compile_normrefl_summary:
    input:
        # Recursively find all txt files in results directory
        files=lambda wildcards: list(Path("data/interim/02_seasim_ts/03A_norm_refl").rglob("*/*_blocks.csv"))
    output:
...
rule ts_RGB_refl_animate:
    input:
        csv_file="data/interim/02_seasim_ts/03A_norm_refl-blocks.csv"
    output:
        out_dir=directory("data/interim/02_seasim_ts/animation")
    params:
        method='bilinear',
        block_sizes=['11x11', '7x7', '3x3', '1x1']
    run:
        import shutil

        ts_roi_blocks = pd.read_csv(input.csv_file,index_col=0)
        ts_roi_blocks = ts_roi_blocks.loc[ts_roi_blocks['resampling_method'] == params.method]
        ts_roi_blocks = ts_roi_blocks.loc[ts_roi_blocks['block_grid'].isin(params.block_sizes)]
        TS_BLOCKS = ts_roi_blocks['filestem'].tolist()
        TS_LABELS = ts_roi_blocks['label'].tolist()

        for block_size in params.block_sizes:  # unique block sizes
            blocked_df = ts_roi_blocks.loc[ts_roi_blocks['block_grid'] == block_size]

            for label in list(dict.fromkeys(TS_LABELS)):
                filestems = blocked_df['filestem'].loc[blocked_df['label'] == label].tolist()
                filestems = [filestem + ".png" for filestem in filestems]
                data_dir = Path(f"data/interim/02_seasim_ts/03A_norm_refl/{label}/")

                png_files = [
                    str(path) for path in Path(data_dir).rglob("*.png")
                    if path.name in filestems and path.is_file()
                ]
                png_files.sort()
                #print(png_files)
                ...
                for source_file in png_files:
                    ...
                    destination_dir = Path(output.out_dir).parent.joinpath(f'figures/{block_size}/{label}')
                    destination_dir.mkdir(parents=True,exist_ok=True)
                    destination_path = destination_dir.joinpath(f"{source_file.name}")
                    shutil.copy2(source_file,destination_path)

                output_gif_path = Path(output.out_dir).joinpath(f'{block_size}')
                if not output_gif_path.exists():
                    output_gif_path.mkdir(parents=True,exist_ok=True)

                images.create_gif(png_files,f"{output_gif_path.joinpath(f"{label}_RGB.gif")}",duration=500,loop=0)
